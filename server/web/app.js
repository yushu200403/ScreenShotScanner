...
function modal(inner) { saveDeviceDraft();if(!$(".modal")){modalReturnFocus=document.activeElement;modalBodyOverflow=document.body.style.overflow;}document.body.style.overflow="hidden";$("#modal-root").innerHTML='<div class="modal-backdrop" data-close-modal><div class="modal" role="dialog" aria-modal="true" tabindex="-1" onclick="event.stopPropagation()"><button class="quiet-button modal-close" type="button" aria-label="关闭弹窗" title="关闭" onclick="closeModal()"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button>'+inner+'</div></div>';const dialog=$(".modal");dialog.setAttribute("aria-label",dialog.querySelector("h3")?.textContent||"设置");(dialog.querySelector("input,textarea,select")||dialog.querySelector("button")||dialog).focus(); }
...
function requestResult(row) {
  ...
  return `<div class="answer-panel" data-answer-id="${esc(row.id)}"><div class="panel-title answer-heading"><div><h4>回答</h4><p>${date(row.created_at)}${row.preset_name?" · "+esc(row.preset_name):""}</p></div><div class="answer-actions">${statusLabel(row.status)}${row.screenshot_available?`<a class="secondary-button screenshot-link" href="${esc(row.screenshot_url)}" target="_blank" rel="noreferrer" data-screenshot-preview aria-label="查看截图" title="查看截图" aria-haspopup="dialog"><i class="ri ri-image" aria-hidden="true"></i></a>`:""}</div></div>
    ...
    <div class="prose answer-prose">${row.answer?renderMarkdown(row.answer):`<p class="muted">${placeholder}</p>`}</div></div>`;
}
