...
旧客户端正在运行而无法覆盖时，可指定 `-OutputName ScreenShotScannerClient-2.1.3.exe` 输出到其他文件；默认文件名不变。
...
版本由 `../server/app/release.json` 统一定义，打包脚本会将它一并打入程序。客户端与服务器版本不一致时会拒绝登录、连接和截图上传，必须同时更新。当前版本为 2.1.3，保留 2.0.0 数据；从 1.x 升级时服务端会清空重建旧版数据表。
