...
连接成功后，电脑信息旁会显示网站二维码，手机扫码即可打开同一服务器的网页并登录。断开或退出登录时二维码收起，重新连接后重新显示。使用 localhost 等仅供本机访问的地址时，会提示改用手机可访问的服务器地址。
...
旧客户端正在运行而无法覆盖时，可指定 `-OutputName ScreenShotScannerClient-2.1.4.exe` 输出到其他文件；默认文件名不变。
...
版本由 `../server/app/release.json` 统一定义，打包脚本会将它一并打入程序。客户端与服务器版本不一致时会拒绝登录、连接和截图上传，必须同时更新。当前版本为 2.1.4，保留 2.0.0 数据；从 1.x 升级时服务端会清空重建旧版数据表。
